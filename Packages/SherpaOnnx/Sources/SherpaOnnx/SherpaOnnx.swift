...
final class SherpaOnnxGenerationConfigC {
  private let swiftConfig: SherpaOnnxGenerationConfigSwift

  /// Keeps reference audio alive while the C API reads it.
  ...
  init(_ swiftConfig: SherpaOnnxGenerationConfigSwift) {
    ...
    self.referenceAudioStorage = swiftConfig.referenceAudio
    self.extraJson = swiftConfig.extraJsonString()
  }

  /// Build `SherpaOnnxGenerationConfig` with valid pointers for the duration of `body`.
  func withUnsafeConfigPointer<R>(
    _ body: (UnsafePointer<SherpaOnnxGenerationConfig>) -> R
  ) -> R {
    referenceAudioStorage.withUnsafeBufferPointer { buffer in
...
class SherpaOnnxOfflineTtsWrapper {
  ...
  func generateWithConfig(
    ...
    let audio: UnsafePointer<SherpaOnnxGeneratedAudio>? = bridge.withUnsafeConfigPointer {
      configPtr in
